import {
  ArgsTable,
  Canvas,
  Description,
  Meta,
  Story
} from '@storybook/addon-docs'

import { terminalContent } from '@/../.storybook/terminal_content'

import ChangeDir, { binary as cdBinary } from '@/bins/ChangeDir.vue'
import Clear from '@/bins/Clear.vue'
import Hello from '@/bins/Hello.vue'
import Help from '@/bins/Help.vue'
import Hostname from '@/bins/Hostname.vue'
import List, { binary as lsBinary } from '@/bins/List.vue'
import Manual, { binary as manBinary } from '@/bins/Manual.vue'
import Tree, { binary as treeBinary } from '@/bins/Tree.vue'
import WhoAmI, { binary as whoamiBinary } from '@/bins/WhoAmI.vue'
import WorkDir from '@/bins/WorkDir.vue'

<Meta
  title="Bins/Callable"
  decorators={[
    terminalContent,
  ]}/>

# Callable binaries

see·el·aye comes with a set of built-in binaries for handling some common CLI
actions. These include the following.

## `hello`

<Description of={Hello}/>

export const helloTemplate = () => ({
  components: { Hello },
  template: `
    <Hello/>
  `
})

<Canvas>
  <Story name="hello">
    {helloTemplate.bind({})}
  </Story>
</Canvas>

## `cd`

<Description of={ChangeDir}/>

export const cdTemplate = (args) => ({
  components: { ChangeDir },
  setup() {
    return { args }
  },
  template: `
    <ChangeDir v-bind="args" />
  `
})

<Canvas>
  <Story
    name="cd"
    argTypes={{
      '--verbose/-v': {
        table: {
          type: { summary: 'boolean' },
          defaultValue: { summary: false },
        },
        description: cdBinary.argSpec.kwArgs[0].description,
        control: null
      },
      'dirpath': {
        table: {
          type: { summary: 'string' },
          defaultValue: { summary: cdBinary.argSpec.posArgs[0].default },
        },
        description: cdBinary.argSpec.posArgs[0].description,
        control: null
      },
    }}
    args={{
      argv: [
        '--verbose',
        '~/c',
      ],
    }}>
    {cdTemplate.bind({})}
  </Story>
</Canvas>

<ArgsTable story="cd"/>

## `clear`

<Description of={Clear}/>

export const clearTemplate = () => ({
  components: { Clear },
  template: `
    <Clear/>
  `
})

<Canvas>
  <Story name="clear">
    {clearTemplate.bind({})}
  </Story>
</Canvas>

## `help`

<Description of={Help}/>

export const helpTemplate = () => ({
  components: { Help },
  template: `
    <Help/>
  `
})

<Canvas>
  <Story name="help">
    {helpTemplate.bind({})}
  </Story>
</Canvas>

## `hostname`

<Description of={Hostname}/>

export const hostnameTemplate = () => ({
  components: { Hostname },
  template: `
    <Hostname/>
  `
})

<Canvas>
  <Story name="hostname">
    {hostnameTemplate.bind({})}
  </Story>
</Canvas>

## `ls`

<Description of={List}/>

export const listTemplate = (args) => ({
  components: { List },
  setup() {
    return { args }
  },
  template: `
    <List v-bind="args" />
  `
})

<Canvas>
  <Story
    name="ls"
    argTypes={{
      '--all/-a': {
        table: {
          type: { summary: 'boolean' },
          defaultValue: { summary: lsBinary.argSpec.kwArgs[0].default },
        },
        description: lsBinary.argSpec.kwArgs[0].description,
        control: null
      },
      'dirpath': {
        table: {
          type: { summary: 'string' },
          defaultValue: { summary: lsBinary.argSpec.posArgs[0].default },
        },
        description: lsBinary.argSpec.posArgs[0].description,
        control: null
      },
    }}
    args={{
      argv: [
        '--all',
        '~',
      ],
    }}>
    {listTemplate.bind({})}
  </Story>
</Canvas>

<ArgsTable story="ls"/>

## `man`

<Description of={Manual}/>

export const manualTemplate = (args) => ({
  components: { Manual },
  setup() {
    return { args }
  },
  template: `
    <Manual v-bind="args" />
  `
})

<Canvas>
  <Story
    name="man"
    argTypes={{
      'bin': {
        table: {
          type: { summary: 'string' },
        },
        description: manBinary.argSpec.posArgs[0].description,
        control: null
      },
    }}
    args={{
      argv: [
        'tree',
      ],
    }}>
    {manualTemplate.bind({})}
  </Story>
</Canvas>

<ArgsTable story="man"/>

## `pwd`

<Description of={WorkDir}/>

export const pwdTemplate = () => ({
  components: { WorkDir },
  template: `
    <WorkDir/>
  `
})

<Canvas>
  <Story name="pwd">
    {pwdTemplate.bind({})}
  </Story>
</Canvas>

## `tree`

<Description of={Tree}/>

export const treeTemplate = (args) => ({
  components: { Tree },
  setup() {
    return { args }
  },
  template: `
    <Tree v-bind="args" />
  `
})

<Canvas>
  <Story
    name="tree"
    argTypes={{
      '--verbose/-v': {
        table: {
          type: { summary: 'boolean' },
          defaultValue: { summary: treeBinary.argSpec.kwArgs[0].default },
        },
        description: treeBinary.argSpec.kwArgs[0].description,
        control: null
      },
      'dirpath': {
        table: {
          type: { summary: 'string' },
          defaultValue: { summary: treeBinary.argSpec.posArgs[0].default },
        },
        description: treeBinary.argSpec.posArgs[0].description,
        control: null
      },
    }}
    args={{
      argv: [
        '--verbose',
        '~',
      ],
    }}>
    {treeTemplate.bind({})}
  </Story>
</Canvas>

<ArgsTable story="tree"/>

## `whoami`

<Description of={WhoAmI}/>

export const whoamiTemplate = (args) => ({
  components: { WhoAmI },
  setup() {
    return { args }
  },
  template: `
    <WhoAmI v-bind="args"/>
  `
})

<Canvas>
  <Story
    name="whoami"
    argTypes={{
      '--groot/-g': {
        table: {
          type: { summary: 'boolean' },
          defaultValue: { summary: whoamiBinary.argSpec.kwArgs[0].default },
        },
        description: whoamiBinary.argSpec.kwArgs[0].description,
        control: null
      },
    }}
    args={{ argv: ['--groot'] }}>
    {whoamiTemplate.bind({})}
  </Story>
</Canvas>

<ArgsTable story="whoami"/>
